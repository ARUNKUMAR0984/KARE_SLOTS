...
require_once('ultramsg.class.php'); // Include the UltraMsg class

$mysqli = new mysqli('localhost', 'root', '', 'libraryseminarhall');
if (isset($_GET['date'])) {
    $date = $_GET['date'];
...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        ...
        $userMail->Body = "Hello <strong>$name</strong>,<br>Your booking for timeslot $timeslots on date $dates at LIBRARY SEMINAR HALL to conduct <strong>$purpose</strong> with <strong>$requirement</strong> has been submitted for approval.<br>Please stay tuned.<br> Thank you for choosing our service!";
        ...
        $dates = date('d-m-Y', strtotime($date));
        $adminMail->Body = "Hello Sir,<br>There is a booking application for timeslot $timeslots on date $dates at LIBRARY SEMINAR HALL to conduct <strong>$purpose</strong> with <strong>$requirement</strong> that has been submitted for approval.<br><br> Click below to approve the booking:<br>";
        ...
        try {
            ...
            $body = "Hello $name, your booking for timeslot $timeslots on date $dates at LIBRARY SEMINAR HALL has been submitted for approval . Thank you for choosing our service!";
            $api = $client->sendChatMessage($phonenumber, $body);
    
            // WhatsApp message sending code to the administrator
            $admin_body = "New booking by $name for timeslot $timeslots on date $dates at LIBRARY SEMINAR HALL. Please review and approve.";
            $admin_api = $client->sendChatMessage('admin_phone_number', $admin_body);
             
            // SMS Gateway Code
$fields = array(
    "message" => "New booking by $name for timeslot $timeslots on date $dates  at LIBRARY SEMINAR HALL. Please review and approve.",
    "language" => "english",
    "route" => "q",
...
if ($err) {
    ...
    echo $response;
            ...
            header("Location: summarylibrary.php?name=$name&email=$email&timeslots=$timeslots&date=$date");
            exit();
        } catch (Exception $e) {
...
<div class="marquee-container">
        <marquee class="marquee-content"><strong>HELLO, <?php echo $user_data['user_name']; ?> BOOK YOUR SLOTS AT LIBRARY SEMINAR HALL</strong></marquee>
    </div>
<center><h2><STRONG>LIBRARY SEMINAR HALL</STRONG></h2></center>
    <div class="container">
                        ...
                        <div class="col-md-12">
                            <form action="" method="post"> 
                                ...
                                <div class="form-group">
                                    ...
                                    <input required type="email" readonly name="email" id="email" class="form-control" value="<?php echo $user_email; ?>">
                                </div>
                                <div class="form-group">
                                <label for="">VENUE</label>
                                <input required type="text" readonly name="venue" id="venue" class="form-control" value="LIBRARY SEMINAR HALL">
                            </div>
                            <div class="form-group">
    ...
    <script>
        $(".book").click(function (){
            ...
            <center></div>
                    ...
                    <a class="btn btn-primary" href="libraryseminarhall.php" id="button">Back to calendar</a><br><br>
            </div></center>
