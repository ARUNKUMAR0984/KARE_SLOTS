...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        ...
        $userMail->Body = "Hello <strong>$name</strong>,<br>Your booking for timeslot $timeslots on date $dates at INDOOR STADIUM to conduct <strong>$purpose</strong> with <strong>$requirements</strong> has been submitted for approval.<br>Please stay tuned.<br> Thank you for choosing our service!";
        ...
        $dates = date('d-m-Y', strtotime($date));
        $adminMail->Body = "Hello Sir,<br>There is a booking application for timeslot $timeslots on date $dates at INDOOR STADIUM to conduct <strong>$purpose</strong> with <strong>$requirement</strong> that has been submitted for approval.<br><br> Click below to approve the booking:<br>";
        ...
        try {
            ...
            $body = "Hello $name, your booking for timeslot $timeslots on date $dates at INDOOR STADIUM has been submitted for approval . Thank you for choosing our service!";
            $api = $client->sendChatMessage($phonenumber, $body);
    
            // WhatsApp message sending code to the administrator
            $admin_body = "New booking by $name for timeslot $timeslots on date $dates at INDOOR STADIUM. Please review and approve.";
            $admin_api = $client->sendChatMessage('admin_phone_number', $admin_body);
             
            // SMS Gateway Code
$fields = array(
    "message" => "New booking by $name for timeslot $timeslots on date $dates to at INDOOR STADIUM. Please review and approve.",
    "language" => "english",
    "route" => "q",
...
if ($err) {
    ...
    echo $response;
            ...
            header("Location: summaryinddor.php?name=$name&email=$email&timeslots=$timeslots&date=$date");
            exit();
        } catch (Exception $e) {
...
<body>
    <div class="marquee-container">
        <marquee class="marquee-content"><strong>HELLO, <?php echo $user_data['user_name']; ?> BOOK YOUR SLOTS AT INDOOR
                STADIUM</strong></marquee>
    </div>
    <br><br>
    <center>
    ...
    <center>
        ...
        <a class="btn btn-primary" href="calender.php" id="button">Back to calendar</a><br><br>
        </div>
    </center>
    <script>
