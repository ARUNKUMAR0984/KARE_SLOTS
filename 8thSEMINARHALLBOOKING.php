...
$mysqli = new mysqli('localhost','root','','seminarhall');
if(isset($_GET['date']))
{
    $date = $_GET['date'];
...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        ...
        $userMail->Body = "Hello <strong>$name</strong>,<br>Your booking for timeslot $timeslots on date $dates at 8TH BLOCK SEMINAR HALL to conduct <strong>$purpose</strong> with <strong>$requirements</strong> has been submitted for approval.<br>Please stay tuned.<br> Thank you for choosing our service!";
        ...
        $dates = date('d-m-Y', strtotime($date));
        $adminMail->Body = "Hello Sir,<br>There is a booking application for timeslot $timeslots on date $dates at 8TH BLOCK SEMINAR HALL to conduct <strong>$purpose</strong> with <strong>$requirement</strong> that has been submitted for approval.<br><br> Click below to approve the booking:<br>";
        ...
        try {
            ...
            $body = "Hello $name, your booking for timeslot $timeslots on date $dates has been confirmed at 8TH BLOCK SEMINAR HALL. Thank you for choosing our service!";
            $api = $client->sendChatMessage($phonenumber, $body);
    
            // WhatsApp message sending code to the administrator
            $admin_body = "New booking by $name for timeslot $timeslots on date $dates at 8TH BLOCK SEMINAR HALL. Please review and approve.";
            $admin_api = $client->sendChatMessage('admin_phone_number', $admin_body); // Replace 'admin_phone_number' with the actual administrator's phone number
      // SMS Gateway Code
$fields = array(
    "message" => "New booking by $name for timeslot $timeslots on date $dates at 8TH COMPLEX SEMINAR HALL-(8004). Please review and approve.",
    "language" => "english",
    "route" => "q",
...
if ($err) {
    ...
    echo $response;
            ...
            header("Location: summary.php?name=$name&email=$email&timeslots=$timeslots&date=$date");
            exit();
        } catch (Exception $e) {
            $msg .= "<div class='alert alert-warning'>Failed to send confirmation email: " . $userMail->ErrorInfo . "</div>";
        }
...
// Function to send a reminder message to the administrator
function sendReminderToAdmin($name, $timeslot, $date, $phonenumber) {
    // SMS Gateway Code
    $fields = array(
        "message" => "Pending booking by $name for timeslot $timeslot on date $date at 8TH COMPLEX SEMINAR HALL-(8004). Please review and approve.",
        "language" => "english",
        "route" => "q",
        "numbers" => "8015130984", // Replace with the administrator's phone number
    ...
    curl_setopt_array($curl, array(
        ...
        CURLOPT_POSTFIELDS => json_encode($fields),
        CURLOPT_HTTPHEADER => array(
            "authorization: YourFast2SMSAPIKey", // Replace with your Fast2SMS API key
            "accept: */*",
            "cache-control: no-cache",
            "content-type: application/json"
        ),
    ...
    if ($err) {
        error_log("SMS Reminder Error: " . $err);
    } else {
        // Log the reminder was sent
        error_log("SMS Reminder Sent to Administrator for Booking by $name");
    }
}

// Query the database for pending bookings older than 8 hours
$eightHoursAgo = strtotime('-3 minutes');
$query = "SELECT * FROM users WHERE status = 'PENDING' AND date < '$eightHoursAgo'";
$result = $mysqli->query($query);

while ($row = $result->fetch_assoc()) {
    // Send a reminder message to the administrator
    sendReminderToAdmin($row['name'], $row['timeslot'], $row['date'], '8015130984');
}
...
<body>
    <div class="marquee-container">
        <marquee class="marquee-content"><strong>HELLO, <?php echo $user_data['user_name']; ?> BOOK YOUR SLOTS AT 8TH
                BLOCK SEMINAR HALL</strong></marquee>
    </div>
    <center>
        <h2><STRONG>8TH BLOCK SEMINAR HALL</STRONG></h2>
    </center>
    ...
    <center>
        ...
        <a class="btn btn-primary" href="8thSEMINARHALL.php" id="button">Back to calendar</a><br><br>
        </div>
    </center>
