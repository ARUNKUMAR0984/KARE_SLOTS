...
$venues = ["seminarhall", "9thseminarhall", "auditorium", "tcseminarhall", "bookingcalender", "libraryseminarhall"];

// Define an associative array to map database names to defined values
$venueMappings = [
...
foreach ($venues as $venue) {
    ...
    $query = "SELECT email, date, timeslot, venue, phonenumber FROM users WHERE email = ?";
    $stmt = $mysqli->prepare($query);

    if ($stmt) {
        $stmt->bind_param("s", $user_email);
        $stmt->execute();
        $stmt->bind_result($email, $date, $timeslot, $venue, $phonenumber);
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['date']) && isset($_GET['timeslot'])) {
    ...
    foreach ($venues as $venue) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                echo '<script>alert("Booking successfully canceled for ' . (isset($venueMappings[$venue]) ? $venueMappings[$venue] : $venue). '.");</script>';
    ...
    if ($cancellationSuccessful) {
        ...
        $mail->Body = "Hello " . $user_data['user_name'] . ",<br>Your booking for timeslot $timeslot on date $date has been canceled at " . (isset($venueMappings[$venue]) ? $venueMappings[$venue] : $venue) . "<br>Thank you for using our service!";
        ...
        try {
            ...
            $ultramsg_token = "dvbpuqo983tqs4r7"; // Ultramsg.com token
            $instance_id = "instance62785"; // Ultramsg.com instance id
            $client = new UltraMsg\WhatsAppApi($ultramsg_token, $instance_id);
            $body = "Hello " . $user_data['user_name'] . ", your booking for timeslot $timeslot on date $date has been canceled at " . (isset($venueMappings[$venue]) ? $venueMappings[$venue] : $venue) . " Thank you for choosing our service!";
...
$fields = array(
    "message" => "Hello $name, your booking for timeslot $timeslots on date $dates has been confirmed at 8TH COMPLEX SEMINAR HALL-(8004).",
    "language" => "english",
    "route" => "q",
